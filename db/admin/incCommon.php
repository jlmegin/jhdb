<?php
	...
	*/
	...
	function getTableList(){
		$arrTables=array(   
			'tblContributors'=>array('tblContributors', '', 'table.gif'),
			'tblEntities'=>array('tblEntities', '', 'table.gif'),
			'tblGalleries'=>array('tblGalleries', '', 'table.gif'),
			'tblAccessions'=>array('tblAccessions', '', 'table.gif'),
			'tblGalleryItems'=>array('tblGalleryItems', '', 'table.gif'),
			'tblContentTypes'=>array('tblContentTypes', '', 'table.gif'),
			'tblSubGalleryTypes'=>array('tblSubGalleryTypes', '', 'table.gif'),
			'tblSources'=>array('tblSources', '', 'table.gif'),
			'tblTemplates'=>array('tblTemplates', '', 'table.gif'),
			'tblNavMenu'=>array('tblNavMenu', '', 'table.gif'),
			'tblUploadLogs'=>array('tblUploadLogs', '', 'table.gif'),
			'tblSidebarEntries'=>array('tblSidebarEntries', '', 'table.gif'),
			'tblKeywords'=>array('tblKeywords', '', 'table.gif')
			);
	...
	function logInMember(){
		...
		if($_POST['signIn']!=''){
			if($_POST['username']!='' && $_POST['password']!=''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('JHDB_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('JHDB_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['JHDB_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['JHDB_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="index.php"><i class="glyphicon glyphicon-home"></i> JHDB</a>
			</div>
