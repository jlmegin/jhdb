	...
	*/
	...
	if(!function_exists('getTableList')){
		function getTableList(){
			$arrTables=array(   
				"tblContributors"=>"tblContributors",
				"tblEntities"=>"tblEntities",
				"tblGalleries"=>"tblGalleries",
				"tblAccessions"=>"tblAccessions",
				"tblGalleryItems"=>"tblGalleryItems",
				"tblContentTypes"=>"tblContentTypes",
				"tblSubGalleryTypes"=>"tblSubGalleryTypes",
				"tblSources"=>"tblSources",
				"tblTemplates"=>"tblTemplates",
				"tblNavMenu"=>"tblNavMenu",
				"tblUploadLogs"=>"tblUploadLogs",
				"tblSidebarEntries"=>"tblSidebarEntries",
				"tblKeywords"=>"tblKeywords"
				);
	...
	function logOutUser(){
		...
		if(isset($_COOKIE['JHDB_rememberMe'])){
			setcookie('JHDB_rememberMe', '', time()-42000);
		}
		session_destroy();
	}
	########################################################################
	...
	function setupMembership(){
		...
		if($adminGroupID){
			...
			$all_tables = array(  'tblContributors' => 0, 'tblEntities' => 0, 'tblGalleries' => 0, 'tblAccessions' => 0, 'tblGalleryItems' => 0, 'tblContentTypes' => 0, 'tblSubGalleryTypes' => 0, 'tblSources' => 0, 'tblTemplates' => 0, 'tblNavMenu' => 0, 'tblUploadLogs' => 0, 'tblSidebarEntries' => 0, 'tblKeywords' => 0);
